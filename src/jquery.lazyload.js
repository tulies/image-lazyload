...
;(function ($, window, document, undefined) {
  ...
  $.fn.lazyload = function (options) {
    ...
    if (settings.event.indexOf('touchmove') === 0) {
      $container.bind(settings.event, function () {
        return update()
      })
    }
    ...
    if (settings.event.indexOf('transitionend') === 0) {
      $container.bind(TRANSITIONEND_EV, function () {
        return update()
      })
    }
    this.each(function () {
      ...
      } else if ($self.css('background-image').length < 6) {
        $self.css('background-image', 'url(' + settings.placeholder + ')')
      }
      /* When appear is triggered load original image. */
      $self.one('appear', function () {
        if (!this.loaded && $self.attr('data-' + settings.data_attribute + '-loaded') != '1') {
          if (settings.appear) {
          ...
          $('<img />')
            .bind('load', function () {
              ...
              self.loaded = true
              $self.attr('data-' + settings.data_attribute + '-loaded', '1')// clean display
